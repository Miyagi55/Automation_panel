    ...
    async def login_account(
        ...
        try:
            ...
            if login_successful:
                ...
                if not sim_success:
                    log_func(
                        f"Feed simulation failed for account {account_id}, keeping browser open for {ACCOUNT_TEST_BROWSER_TIMEOUT_SECONDS} seconds"
                    )
                ...
                log_func(
                    f"Skipping feed simulation for account {account_id} due to login failure"
                )
                log_func(
                    f"Keeping browser open for {ACCOUNT_TEST_BROWSER_TIMEOUT_SECONDS} seconds for manual inspection for account {account_id}"
                )
                await Randomizer.sleep(
                    ACCOUNT_TEST_BROWSER_TIMEOUT_SECONDS,
                    ACCOUNT_TEST_BROWSER_TIMEOUT_SECONDS,
            ...
            if browser and not (hasattr(browser, "_closed") and browser._closed):
                ...
                if playwright_instance:
                    try:
                        ...
                        log_func(
                            f"Stopped Playwright instance for account {account_id}"
                        )
                    except Exception as e:
                        log_func(
                            f"Error stopping Playwright instance for account {account_id}: {str(e)}"
                        )
