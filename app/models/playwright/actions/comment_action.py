...
from app.utils.config import COMMENT_ACTION_DELAY_RANGE
...
class CommentAction(AutomationAction):
    ...
    async def execute(
        ...
        try:
            ...
            overlay = await self._wait_for_post_overlay(
            ...
            if overlay:
                ...
                await Randomizer.sleep(*COMMENT_ACTION_DELAY_RANGE)
                comment_field = await self._find_comment_field(
                    overlay, debug, account_id, log_func
                )
            else:
                log_func(
        ...
        for attempt in range(3):
            for selector in comment_selectors:
                try:
                    ...
                    log_func(
                        f"Attempt {attempt + 1}: Selector {selector} not found for account {account_id}"
                    )
            if attempt < 2:
