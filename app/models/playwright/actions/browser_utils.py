        ...
        try:
            if browser and not (hasattr(browser, "_closed") and browser._closed):
                ...
                if not self.browser_manager.get_chromium_executable(self.log_func):
                    raise RuntimeError(
                        f"No chromium executable found for account {self.account_id}"
                    )
            ...
            self.log_func(
                f"Failed to initialize browser for account {self.account_id}: {str(e)}"
            )
            raise
    ...
    async def cleanup_browser(
        ...
        try:
            ...
            if playwright:
                ...
                self.log_func(
                    f"Closed Playwright instance for account {self.account_id}"
                )
        except Exception as e:
            self.log_func(
                f"Error during browser cleanup for account {self.account_id}: {str(e)}"
            )

    async def scroll_element(
        self, element: Any, name: str, distance: int, debug: bool
    ) -> None:
        ...
        try:
            ...
            if scroll_after <= scroll_before and debug:
                self.log_func(
                    f"No scrollable element responded for account {self.account_id}"
                )
        except Exception as e:
            if debug:
